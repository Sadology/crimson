            ...
            async function fetchGuildData(){
                await Guild.findOne({
                    ...
                    if(res){
                        ...
                        const { MinExp, MaxExp, ExpMulti, ExpCD, LvlupMsg, Channel, NoExpRole } = res.RankSettings
                        ...
                        noexp = NoExpRole ? NoExpRole : "noexp"
                    }
                }).catch(err => {return console.log(err.stack)});

                if(Module == false) {return
                }else {
                    let expblock = message.guild.roles.resolve(noexp)
                    if(expblock){
                        if(message.member.roles.cache.has(expblock.id)){
                            return
                        }else {
                            coolDown()
                            checkRole()
